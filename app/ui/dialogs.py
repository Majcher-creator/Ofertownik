...
class CompanyProfilesDialog(tk.Toplevel):
    """
    Dialog for managing company profiles.
    Allows viewing, adding, editing, deleting, and selecting company profiles.
    """
    
    def __init__(self, parent, profiles_path: str):
        """
        Initialize the company profiles dialog.
        
        Args:
            parent: Parent window
            profiles_path: Path to the company_profiles.json file
        """
    ...
    def _save_profiles(self):
        ...
        try:
            ...
            messagebox.showerror("Błąd", f"Nie można zapisać profili: {e}")
    ...
    def _create_widgets(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Zarządzanie profilami firmy", 
                               font=('Segoe UI', 12, 'bold'))
        title_label.pack(pady=(0, 10))
        
        # Profiles list
        list_frame = ttk.LabelFrame(main_frame, text="Dostępne profile", padding=10)
        ...
        self.profiles_listbox = tk.Listbox(list_frame, yscrollcommand=scrollbar.set,
        ...
        self.profiles_listbox.bind('<Double-Button-1>', lambda e: self._load_selected())
        ...
        ttk.Button(button_frame, text="Nowy profil", 
                  command=self._add_profile).pack(side='left', padx=2)
        ttk.Button(button_frame, text="Edytuj", 
                  command=self._edit_profile).pack(side='left', padx=2)
    ...
    def _refresh_list(self):
        ...
        for profile in self.profiles:
            name = profile.get('profile_name', 'Bez nazwy')
    ...
    def _edit_profile(self):
        ...
        if not selection:
            messagebox.showwarning("Uwaga", "Wybierz profil do edycji")
            return
        ...
        # Open edit dialog
        dlg = CompanyEditDialog(self, f"Edytuj profil: {profile.get('profile_name', '')}", 
                               profile.copy())
        if getattr(dlg, 'result', None):
            # Update profile but keep profile_name
            profile_name = profile.get('profile_name', '')
            self.profiles[idx].update(dlg.result)
            self.profiles[idx]['profile_name'] = profile_name
            self._save_profiles()
            self._refresh_list()
    ...
    def _delete_profile(self):
        ...
        selection = self.profiles_listbox.curselection()
        if not selection:
            messagebox.showwarning("Uwaga", "Wybierz profil do usunięcia")
            return
        
        idx = selection[0]
        profile = self.profiles[idx]
        name = profile.get('profile_name', 'ten profil')
    ...
    def _load_selected(self):
        """Load selected profile and close dialog."""
        selection = self.profiles_listbox.curselection()
        if not selection:
            messagebox.showwarning("Uwaga", "Wybierz profil do wczytania")
            return
        
        idx = selection[0]
        self.selected_profile = self.profiles[idx].copy()
        self.destroy()
